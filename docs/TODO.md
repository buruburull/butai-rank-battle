...
### 27. シャードショップシステム（完了）
- [x] 通貨「シャード(Shard)」— MOB討伐・鉱石採掘でEPと同時に獲得
- [x] ether_growthテーブルにshards/shards_totalカラム追加
- [x] player_upgradesテーブル新設（永続強化管理）
- [x] EtherGrowthDAO: シャード読み書き・永続強化CRUD
- [x] ShopManager: 商品定義・購入処理・永続強化ロジック
- [x] ShopGUI: チェストGUIでショップ画面（消耗品/スキル/永続強化）
- [x] ShopGUIListener: 購入クリックハンドラ
- [x] シャード獲得（鉱石: 1-10S、MOB: 2-20S）
- [x] 消耗品: ダイヤ剣/ピッケル、回復ポーション、金リンゴ、鉄装備セット
- [x] スキルアイテム: パワーチャージ(攻撃+3/30秒)、オーバードライブ(攻撃+6/15秒)
- [x] SkillItemListener: 右クリック発動、クールダウン管理、タワー退出時クリーンアップ
- [x] 永続強化: エーテル上限+10（最大10回、500S〜×1.5倍）、タワー体力+1♥（最大5回、600S〜×1.5倍）
- [x] タワー入場時にボーナスHP適用、退出時にリセット
- [x] タワー各階にショップNPC配置（ARMORER職業）
- [x] frames.ymlにshop_npcs座標設定
